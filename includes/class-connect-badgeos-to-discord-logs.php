 ...
 */
class Connect_Badgeos_To_Discord_Logs {
	function __construct() {
		// Clear all existing logs.
		add_action( 'wp_ajax_ets_badgeos_discord_clear_logs', array( $this, 'ets_badgeos_discord_clear_logs' ) );
	}
	 ...
	 */
	public static $log_file_name = 'badgeos_discord_api_logs.txt';
	 ...
	 * @return None
	 */
	public function ets_badgeos_discord_clear_logs() {
	...
	static function write_api_response_logs( $response_arr, $user_id, $backtrace_arr = array() ) {
		...
		$log_api_response = sanitize_text_field( trim( get_option( 'ets_badgeos_discord_log_api_response' ) ) );
		$uuid             = sanitize_text_field( trim( get_option( 'ets_badgeos_discord_uuid_file_name' ) ) );
...
new Connect_Badgeos_To_Discord_Logs();
