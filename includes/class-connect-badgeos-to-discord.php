 ...
 */
class Connect_Badgeos_To_Discord {
	 ...
	 * @since    1.0.0
	 * @access   protected
	 * @var      Connect_Badgeos_To_Discord_Loader    $loader    Maintains and registers all hooks for the plugin.
	 */
	protected $loader;
	...
	public function __construct() {
		if ( defined( 'CONNECT_BADGEOS_TO_DISCORD_VERSION' ) ) {
			$this->version = CONNECT_BADGEOS_TO_DISCORD_VERSION;
		} else {
			$this->version = '1.0.0';
		}
		$this->plugin_name = 'connect-badgeos-to-discord';
	 ...
	 *
	 * - Connect_Badgeos_To_Discord_Loader. Orchestrates the hooks of the plugin.
	 * - Connect_Badgeos_To_Discord_i18n. Defines internationalization functionality.
	 * - Connect_Badgeos_To_Discord_Admin. Defines all hooks for the admin area.
	 * - Connect_Badgeos_To_Discord_Public. Defines all hooks for the public side of the site.
	 *
	...
	private function load_dependencies() {
		...
		* The class responsible for defining all methods that help to schedule actions.
		 ...
		 * core plugin.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-connect-badgeos-to-discord-loader.php';

		/**
		 * The class responsible for defining internationalization functionality
		 * of the plugin.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-connect-badgeos-to-discord-i18n.php';

		/**
		 * The class responsible for defining all actions that occur in the admin area.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/class-connect-badgeos-to-discord-admin.php';
	 ...
	 *
	 * Uses the Connect_Badgeos_To_Discord_i18n class in order to set the domain and to register the hook
	 * with WordPress.
	 *
	 * @since    1.0.0
	 * @access   private
	...
	private function set_locale() {
		...
		$plugin_i18n = new Connect_Badgeos_To_Discord_i18n();
	...
	private function define_admin_hooks() {
		...
		$this->loader->add_action( 'admin_init', $plugin_admin, 'ets_badgeos_discord_action_connect_bot' );
	...
	private function define_public_hooks() {
		...
		$this->loader->add_action( 'wp_ajax_badgeos_disconnect_from_discord', $plugin_public, 'ets_badgeos_discord_disconnect_from_discord' );
	...
	private function define_common_hooks() {
		$this->loader->add_action( 'action_scheduler_failed_execution', $this, 'ets_badgeos_discord_reschedule_failed_action' );
		$this->loader->add_filter( 'action_scheduler_queue_runner_batch_size', $this, 'ets_badgeos_discord_queue_batch_size' );
		$this->loader->add_filter( 'action_scheduler_queue_runner_concurrent_batches', $this, 'ets_badgeos_discord_concurrent_batches' );
	 ...
	 * @return NONE
	 */
	public function ets_badgeos_discord_reschedule_failed_action( $action_id ) {
		// First check if the action is for badgeos discord.
		$action_data = ets_badgeos_discord_as_get_action_data( $action_id );
		if ( $action_data !== false ) {
			...
			$hook_failed_count = ets_badgeos_discord_count_of_hooks_failures( $hook );
			$retry_api_count   = absint( sanitize_text_field( trim( get_option( 'ets_badgeos_discord_retry_api_count' ) ) ) );
			if ( $hook_failed_count < $retry_api_count && $retry_failed_api == true && $action_data['as_group'] == BADGEOS_DISCORD_AS_GROUP_NAME && $action_data['status'] === 'failed' ) {
				as_schedule_single_action( ets_badgeos_discord_get_random_timestamp( ets_badgeos_discord_get_highest_last_attempt_timestamp() ), $hook, array_values( $args ), BADGEOS_DISCORD_AS_GROUP_NAME );
			}
		}
	}
	 ...
	 */
	public function ets_badgeos_discord_queue_batch_size( $batch_size ) {
		if ( ets_badgeos_discord_get_all_pending_actions() !== false ) {
			return absint( get_option( 'ets_badgeos_discord_job_queue_batch_size' ) );
		} else {
			return $batch_size;
		}
	}
	 ...
	 * @return INT $concurrent_batches
	 */
	public function ets_badgeos_discord_concurrent_batches( $concurrent_batches ) {
		if ( ets_badgeos_discord_get_all_pending_actions() !== false ) {
			return absint( get_option( 'ets_badgeos_discord_job_queue_concurrency' ) );
		} else {
	 ...
	 *
	 * @since     1.0.0
	 * @return    Connect_Badgeos_To_Discord_Loader    Orchestrates the hooks of the plugin.
	 */
